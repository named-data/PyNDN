...
static PyObject *
ExclusionFilter_names_to_ndn(PyObject *py_obj_Names)
{
	PyObject *py_iterator = NULL, *py_item = NULL;
	PyObject *py_exclude, *py_o;
	struct ndn_charbuf *exclude, *name;
	int r;
	...
	py_exclude = NDNObject_New_charbuf(EXCLUSION_FILTER, &exclude);
	JUMP_IF_NULL(py_exclude, error);

	if (py_obj_Names == Py_None)
		return py_exclude;

	r = ndn_charbuf_append_tt(exclude, NDN_DTAG_Exclude, NDN_DTAG);
	...
	while ((py_item = PyIter_Next(py_iterator))) {
		...
		if (type == 0) {
			py_o = PyObject_GetAttrString(py_item, "ndn_data");
			JUMP_IF_NULL(py_o, error);

			if (!NDNObject_IsValid(NAME, py_o)) {
				Py_DECREF(py_o);
			...
			/* append without NDN name tag */
			assert(name->length >= 4);
			r = ndn_charbuf_append(exclude, name->buf + 1, name->length - 2);
			Py_DECREF(py_o);
			JUMP_IF_NEG_MEM(r, error);
		} else if (type == 1) {
			r = ndn_charbuf_append_tt(exclude, NDN_DTAG_Any, NDN_DTAG);
			JUMP_IF_NEG_MEM(r, error);

			r = ndn_charbuf_append_closer(exclude);
			JUMP_IF_NEG_MEM(r, error);
		} else {
	...
	r = ndn_charbuf_append_closer(exclude); /* </Exclude> */
	JUMP_IF_NEG_MEM(r, error);
...
Exclusion_Name_Obj(unsigned char *buf, size_t start, size_t stop)
{
	...
	r = ndn_name_append_components(name, buf, start, stop);
	JUMP_IF_NEG_MEM(r, error);

	res = Name_obj_from_ndn(py_name);
	Py_DECREF(py_name);
...
static int
process_int_attribute(struct ndn_charbuf *interest, enum ndn_dtag tag,
		PyObject *py_obj_Interest, const char *attr_name)
{
	...
	val = _pyndn_Int_AsLong(py_attr);
	Py_DECREF(py_attr);
	if (PyErr_Occurred())
		return -1;

#if 0
	r = ndn_charbuf_append_tt(interest, tag, NDN_DTAG);
	JUMP_IF_NEG_MEM(r, error);

	r = ndnb_append_number(interest, val);
	JUMP_IF_NEG_MEM(r, error);

	r = ndn_charbuf_append_closer(interest); /* </Tag> */
	JUMP_IF_NEG_MEM(r, error);
#endif

	r = ndnb_tagged_putf(interest, tag, "%d", val);
	JUMP_IF_NEG_MEM(r, error);
...
static PyObject *
Interest_obj_to_ndn(PyObject *py_obj_Interest)
{
	struct ndn_charbuf *interest;
	PyObject *py_interest, *py_o;
	int r;

	py_interest = NDNObject_New_charbuf(INTEREST, &interest);
	if (!py_interest)
		return NULL;

	r = ndn_charbuf_append_tt(interest, NDN_DTAG_Interest, NDN_DTAG);
	JUMP_IF_NEG_MEM(r, error);
	...
	/* Name */
	{
		...
		if (r) {
			py_name = Name_obj_to_ndn(py_o);
			Py_DECREF(py_o);
			JUMP_IF_NULL(py_name, error);
			name = NDNObject_Get(NAME, py_name);

			r = ndn_charbuf_append_charbuf(interest, name);
			Py_DECREF(py_name);
			JUMP_IF_NEG_MEM(r, error);
		} else {
			...
			r = ndn_charbuf_append_tt(interest, NDN_DTAG_Name, NDN_DTAG);
			JUMP_IF_NEG(r, error);

			r = ndn_charbuf_append_closer(interest); /* </Name> */
			JUMP_IF_NEG(r, error);
		}
	}

	r = process_int_attribute(interest, NDN_DTAG_MinSuffixComponents,
			py_obj_Interest, "minSuffixComponents");
	JUMP_IF_NEG(r, error);

	r = process_int_attribute(interest, NDN_DTAG_MaxSuffixComponents,
	...
	if (r) {
		...
		r = ndnb_append_tagged_blob(interest, NDN_DTAG_PublisherPublicKeyDigest,
				blob, blobsize);
		Py_DECREF(py_o);
		JUMP_IF_NEG_MEM(r, error);
	}
	...
	r = process_int_attribute(interest, NDN_DTAG_AnswerOriginKind,
			py_obj_Interest, "answerOriginKind");
	JUMP_IF_NEG(r, error);

	r = process_int_attribute(interest, NDN_DTAG_Scope, py_obj_Interest,
			"scope");
	JUMP_IF_NEG(r, error);
	...
	if (r) {
		...
		r = ndnb_append_tagged_blob(interest, NDN_DTAG_InterestLifetime,
				buf, sizeof(buf));
		JUMP_IF_NEG_MEM(r, error);
	}
	...
	if (r) {
		...
		r = ndnb_append_tagged_blob(interest, NDN_DTAG_Nonce, s, len);
		Py_DECREF(py_o);
		JUMP_IF_NEG_MEM(r, error);
	}

	r = ndn_charbuf_append_closer(interest); /* </Interest> */
	JUMP_IF_NEG_MEM(r, error);
...
PyObject *
Interest_obj_from_ndn(PyObject *py_interest)
{
	struct ndn_charbuf *interest;
	...
	debug("Interest_from_ndn_parsed start\n");
	...
	// 2) Set ndn_data to a cobject pointing to the c struct
	//    and ensure proper destructor is set up for the c object.
	r = PyObject_SetAttrString(py_obj_Interest, "ndn_data", py_interest);
	JUMP_IF_NEG(r, error);
	...
	// Best decoding examples are in packet-ndn.c for wireshark plugin?

	//        self.name = None  # Start from None to use for templates?
	len = pi->offset[NDN_PI_E_Name] - pi->offset[NDN_PI_B_Name];
	if (len > 0) {
		PyObject *py_cname;

		py_cname = NDNObject_New_charbuf(NAME, &cb);
		JUMP_IF_NULL(py_cname, error);

		r = ndn_charbuf_append(cb, interest->buf + pi->offset[NDN_PI_B_Name],
				len);
		JUMP_IF_NEG_MEM(r, error);

		py_o = Name_obj_from_ndn(py_cname);
		Py_DECREF(py_cname);
		JUMP_IF_NULL(py_o, error);
		...
		PyErr_SetString(g_PyExc_NDNInterestError, "Got interest without a"
				" name!");
		goto error;
	}

	//        self.minSuffixComponents = None  # default 0
	len = pi->offset[NDN_PI_E_MinSuffixComponents] -
			pi->offset[NDN_PI_B_MinSuffixComponents];
	if (len > 0) {
		r = ndn_fetch_tagged_nonNegativeInteger(NDN_DTAG_MinSuffixComponents,
				interest->buf, pi->offset[NDN_PI_B_MinSuffixComponents],
				pi->offset[NDN_PI_E_MinSuffixComponents]);
		if (r < 0) {
			PyErr_SetString(g_PyExc_NDNInterestError, "Invalid"
	...
	len = pi->offset[NDN_PI_E_MaxSuffixComponents] -
			pi->offset[NDN_PI_B_MaxSuffixComponents];
	if (len > 0) {
		r = ndn_fetch_tagged_nonNegativeInteger(NDN_DTAG_MaxSuffixComponents,
				interest->buf, pi->offset[NDN_PI_B_MaxSuffixComponents],
				pi->offset[NDN_PI_E_MaxSuffixComponents]);
		if (r < 0) {
			PyErr_SetString(g_PyExc_NDNInterestError, "Invalid"
	...
	start = pi->offset[NDN_PI_B_PublisherID];
	end = pi->offset[NDN_PI_E_PublisherID];
	len = end - start;
	if (len > 0) {
		r = ndn_ref_tagged_BLOB(NDN_DTAG_PublisherPublicKeyDigest,
	...
	//        self.exclude = None
	len = pi->offset[NDN_PI_E_Exclude] - pi->offset[NDN_PI_B_Exclude];
	if (len > 0) {
		PyObject *py_exclusion_filter;

		py_exclusion_filter = NDNObject_New_charbuf(EXCLUSION_FILTER, &cb);
		JUMP_IF_NULL(py_exclusion_filter, error);

		r = ndn_charbuf_append(cb, interest->buf + pi->offset[NDN_PI_B_Exclude],
				len);
		JUMP_IF_NEG_MEM(r, error);
	...
	len = pi->offset[NDN_PI_E_ChildSelector] -
			pi->offset[NDN_PI_B_ChildSelector];
	if (len > 0) {
		r = ndn_fetch_tagged_nonNegativeInteger(NDN_DTAG_ChildSelector,
				interest->buf, pi->offset[NDN_PI_B_ChildSelector],
				pi->offset[NDN_PI_E_ChildSelector]);
		if (r < 0) {
			PyErr_SetString(g_PyExc_NDNInterestError, "Invalid"
					" ChildSelector value");
			goto error;
	...
	len = pi->offset[NDN_PI_E_AnswerOriginKind] -
			pi->offset[NDN_PI_B_AnswerOriginKind];
	if (len > 0) {
		r = ndn_fetch_tagged_nonNegativeInteger(NDN_DTAG_AnswerOriginKind,
				interest->buf, pi->offset[NDN_PI_B_AnswerOriginKind],
				pi->offset[NDN_PI_E_AnswerOriginKind]);
		if (r < 0) {
			PyErr_SetString(g_PyExc_NDNInterestError, "Invalid"
	...
	//        self.scope  = None
	len = pi->offset[NDN_PI_E_Scope] - pi->offset[NDN_PI_B_Scope];
	if (len > 0) {
		r = ndn_fetch_tagged_nonNegativeInteger(NDN_DTAG_Scope, interest->buf,
				pi->offset[NDN_PI_B_Scope], pi->offset[NDN_PI_E_Scope]);
		if (r < 0) {
			PyErr_SetString(g_PyExc_NDNInterestError, "Invalid"
					" Scope value");
			goto error;
	...
	len = pi->offset[NDN_PI_E_InterestLifetime] -
			pi->offset[NDN_PI_B_InterestLifetime];
	if (len > 0) {
		double lifetime;

		// From packet-ndn.c
		r = ndn_ref_tagged_BLOB(NDN_DTAG_InterestLifetime, interest->buf,
				pi->offset[NDN_PI_B_InterestLifetime],
				pi->offset[NDN_PI_E_InterestLifetime], &blob, &blob_size);
		if (r < 0) {
			PyErr_SetString(g_PyExc_NDNInterestError, "Invalid"
	...
	//        self.nonce = None
	len = pi->offset[NDN_PI_E_Nonce] - pi->offset[NDN_PI_B_Nonce];
	if (len > 0) {
		r = ndn_ref_tagged_BLOB(NDN_DTAG_Nonce, interest->buf,
				pi->offset[NDN_PI_B_Nonce], pi->offset[NDN_PI_E_Nonce], &blob,
				&blob_size);
		if (r < 0) {
			PyErr_SetString(g_PyExc_NDNInterestError, "Invalid"
					" Nonce value");
...
_pyndn_interest_get_pi(PyObject *py_interest)
{
	struct interest_data *context;
	struct ndn_charbuf *interest;
	int r;

	assert(NDNObject_IsValid(INTEREST, py_interest));
	...
	context->pi = calloc(1, sizeof(struct ndn_parsed_interest));
	JUMP_IF_NULL_MEM(context->pi, error);

	/* TODO: we should also use the comps argument */
	r = ndn_parse_interest(interest->buf, interest->length, context->pi, NULL);
	if (r < 0) {
		PyErr_SetString(g_PyExc_NDNInterestError, "Unable to parse the"
				" Interest");
		goto error;
	}
...
void
_pyndn_interest_set_pi(PyObject *py_interest, struct ndn_parsed_interest *pi)
{
	struct interest_data *context;
...
PyObject *
_pyndn_cmd_Interest_obj_to_ndn(PyObject *UNUSED(self), PyObject *py_obj_Interest)
{
	...
	return Interest_obj_to_ndn(py_obj_Interest);
}

PyObject *
_pyndn_cmd_Interest_obj_from_ndn(PyObject *UNUSED(self), PyObject *py_interest)
{
	if (!NDNObject_IsValid(INTEREST, py_interest)) {
		PyErr_SetString(PyExc_TypeError, "Must pass a NDN Interest as 1st"
				" argument");
		return NULL;
	}

	return Interest_obj_from_ndn(py_interest);
}

PyObject *
_pyndn_cmd_ExclusionFilter_names_to_ndn(PyObject *UNUSED(self), PyObject *py_names)
{
	if (!PyList_Check(py_names)) {
		PyErr_SetString(PyExc_TypeError, "Must pass a list of NDN names");
		return NULL;
	}
